...
#define ENCODER_RESOLUTION 187.95 
  ...
  if (Serial.available()) {
    ...
    if (data.startsWith("V,")) {
      ...
      if (commaIndex1 != -1 && commaIndex2 != -1 && commaIndex3 != -1) {
        int fl_speed = constrain(data.substring(0, commaIndex1).toInt(), -255, 255);
        int fr_speed = constrain(data.substring(commaIndex1 + 1, commaIndex2).toInt(), -255, 255);
        int bl_speed = constrain(data.substring(commaIndex2 + 1, commaIndex3).toInt(), -255, 255);
        int br_speed = constrain(data.substring(commaIndex3 + 1).toInt(), -255, 255);
        
        setMotorSpeed(L298N_in1, L298N_in2, L298N_enFL, fl_speed);
        setMotorSpeed(L298N_in3, L298N_in4, L298N_enBL, bl_speed);
        setMotorSpeed(L298N_in5, L298N_in6, L298N_enFR, fr_speed);
        setMotorSpeed(L298N_in7, L298N_in8, L298N_enBR, br_speed);
...
void stopMotors() {
  setMotorSpeed(L298N_in1, L298N_in2, L298N_enFL, 0);
  setMotorSpeed(L298N_in3, L298N_in4, L298N_enBL, 0);
  setMotorSpeed(L298N_in5, L298N_in6, L298N_enFR, 0);
  setMotorSpeed(L298N_in7, L298N_in8, L298N_enBR, 0);
}
