#define LEFT_MOTOR_PIN1 7    // Direction pin 1 for left motor
#define LEFT_MOTOR_PIN2 6    // Direction pin 2 for left motor
#define LEFT_MOTOR_PWM 5     // PWM pin for left motor

#define RIGHT_MOTOR_PIN1 12  // Direction pin 1 for right motor
#define RIGHT_MOTOR_PIN2 11  // Direction pin 2 for right motor
#define RIGHT_MOTOR_PWM 10   // PWM pin for right motor

#define LEFT_encoder_phaseA 18
#define LEFT_encoder_phaseB 19
#define RIGHT_encoder_phaseA 21
#define RIGHT_encoder_phaseB 20
...
const unsigned long ODOM_INTERVAL = 50; // ms

// Tune to your encoders
#define ENCODER_RESOLUTION 187.95 // counts per revolution (1x on A rising edges)
#define WHEEL_RADIUS 0.070 // m
#define WHEEL_DISTANCE 0.360 // m
    ...
    if (Serial.available()) {
        ...
        if (data.startsWith("V,")) {
            ...
            if (comma > 0) {
                ...
                // Use corrected motor control functions
...
// CORRECTED: Separate direction and PWM pins
void setLeftMotorSpeed(int speed) {
    ...
    digitalWrite(LEFT_MOTOR_PIN1, forward ? HIGH : LOW);
    digitalWrite(LEFT_MOTOR_PIN2, forward ? LOW : HIGH);
    analogWrite(LEFT_MOTOR_PWM, pwm);  // Use dedicated PWM pin
}
...
void setRightMotorSpeed(int speed) {
    ...
    bool forward = speed > 0;
    digitalWrite(RIGHT_MOTOR_PIN1, forward ? HIGH : LOW);
    digitalWrite(RIGHT_MOTOR_PIN2, forward ? LOW : HIGH);
    analogWrite(RIGHT_MOTOR_PWM, pwm);  // Use dedicated PWM pin
}

// CORRECTED: Proper stop function
void stopMotors() {
    digitalWrite(LEFT_MOTOR_PIN1, LOW);
    digitalWrite(LEFT_MOTOR_PIN2, LOW);
    digitalWrite(RIGHT_MOTOR_PIN1, LOW);
